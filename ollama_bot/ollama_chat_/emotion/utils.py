...
def is_neutral_or_decayed_reason(text: str | None) -> bool:
    """理由テキストが平常・時間減衰・未初期化などのニュートラル系であるかを判定する。"""
    cleaned = str(text or "").strip()
    if not cleaned:
        return True
    neutral_patterns = (
    ...
    return any(p in cleaned for p in neutral_patterns)


def is_neutral_or_decayed_appraisal(text: str | None) -> bool:
    """出来事評価テキストが平常・時間減衰・未初期化などのニュートラル系であるかを判定する。"""
    cleaned = str(text or "").strip()
    if not cleaned:
        return True
    neutral_patterns = (
