...
from ...common.emotion_helpers import EMOTION_KEYS, clamp_score
from ..ollama_chat_types import UserRelationship

log = logging.getLogger("ollama_bot.ollama_chat")

from .utils import _load_persisted_timestamp
    ...
    async def _load_emotion_state(self) -> None:
        ...
        try:
            ...
            if dominant == "neutral" or elapsed_hours >= 2.0:
                ...
                self.emotion_state.appraisal = str(saved_state.get("appraisal", "") or "").strip() or "平常状態"
                self.emotion_state.reason = str(saved_state.get("reason", "") or "").strip() or "平常状態"
